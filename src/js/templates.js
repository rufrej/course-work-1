...
export function buildTemplateTodo({id, title, description, createdAt, userName, status}) {
  ...
  return `
         <div class="todo-task"  data-id="${id}"">
          <h4 class="edit-title hidden">Edit Task</h4>
            <div class="todo-task-header">
               <h4 class="todo-task-title">${title}</h4>
                 <input class="hidden" id="task-edit-title" value="${title}" />
            ...
            <p class="todo-task-description">${description}</p>
             <textarea rows="3" id="task-edit-description" class="hidden">${description}</textarea>
            <div class="todo-task-info">
              <p class="todo-task-user-name">${userName}</p>
              <p class="todo-task-time">${createdAt}</p>
            </div>
            <div class="task-setting">
             <select class="form-select task-select" data-role="select-status">
              ...
              <button type="button" class="btn btn-secondary button-close-edit hidden">Close</button>
